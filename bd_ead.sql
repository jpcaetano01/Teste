...
values(default, 'Design'),		-- tabela contem 2 campos código e descrição da categoria.values só escreve 1 vez
(default, 'Infraestrutura'),	-- default campo código da categoria preenchido automatico
(default, 'Dados'),
(default, 'Front_end'),
(default, 'Mobile');
...
(default, 'Sergio Lopes'),						-- codigo 16
...
'<p>A nova certificação MCSA (Microsoft Certified Solutions Associate) é um título atribuído a profissionais da área de TI que
 ...
 create view vw_livro -- criação de uma view 2°
 as select -- select gigantesco para trazer todas as informações do banco de dados 1°
		tbl_livro.cd_livro,
        tbl_livro.no_isbn,
...
inner join tbl_categoria
	on tbl_livro.cd_categoria = tbl_categoria.cd_categoria; -- fim select
    
	select * from vw_livro; -- ao invés de digitar tudo de novo, cria a view acima para depois só executar esse comando
	
    select nm_livro,vl_preco,ds_capa from vw_livro; -- carrega apenas 3 informações
    
    select nm_livro,vl_preco,ds_capa from vw_livro where sg_lancamento = 'S'; -- informar apenas os livros lançamento